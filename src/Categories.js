...
export default function Categories({location}) {
   ...
   const [categorie,setcategorie]=useState(querystring.parse(location.search).id);
...
const RedirecttoPost=(picture,id)=>{
   ...
   const loaddata=()=>{
      setlodign(true)
       fetch("https://javascriptdeveloper.xyz/wp-json/wp/v2/posts?categories="+categorie+"&per_page=10&page="+currentpage+"&_embed")
        .then(res=>res.json())
